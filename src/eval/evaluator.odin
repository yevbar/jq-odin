...
frame_phase :: enum u8 {
	...
	Static_Slice_Active,
	Iterator_Active,
	Iterator_Update_Active,
	Iterator_Update_Child_Active,
	...
	Static_Index_Field_Update_Child_Active,
	Static_Index_Field_Update_Empty,
	Static_Field_Index_Field_Update_Child_Active,
	Static_Field_Index_Field_Update_Empty,
	Binding_Start_Left,
	Binding_Left_Active,
	Binding_Body_Active,
	...
	Path_Assign_Apply,
	Dynamic_Index_Assign_Key_Active,
	Dynamic_Index_Assign_RHS_Start,
	Dynamic_Index_Assign_RHS_Active,
	Dynamic_Index_Assign_Apply,
	Complete,
	Debug_Emit,
	Debug_Output,
	Input_Active,
...
eval_frame :: struct {
	...
	saved_operands:    [4]program.Operand,
	...
	static_index_field_update_seen: bool,
	static_index_field: int,
	static_index_field_in_range: bool,
	static_index_field_root_null: bool,
	static_field_index_field_update_seen: bool,
	static_field_index_field: int,
	static_field_index_field_in_range: bool,
	static_field_index_field_root_null: bool,
	static_field_index_field_missing: bool,
...
@(private)
static_field_update_operands :: proc(storage: ^evaluator_storage, instruction: program.Instruction) -> (key: string, rhs: program.Instruction_Index, ok: bool) {
	if instruction.opcode != .Static_Field_Update || instruction.operands_count != 2 do return
	key_operand, key_ok := program.program_operand(storage.compiled, instruction.operands_start)
...
@(private)
static_index_field_update_operands :: proc(storage: ^evaluator_storage, instruction: program.Instruction) -> (index_text, key: string, rhs: program.Instruction_Index, ok: bool) {
	if instruction.opcode != .Static_Index_Field_Update || instruction.operands_count != 3 do return
	index_operand, index_ok := program.program_operand(storage.compiled, instruction.operands_start)
	key_operand, key_ok := program.program_operand(storage.compiled, program.Operand_Index(u32(instruction.operands_start)+1))
	rhs_operand, rhs_ok := program.program_operand(storage.compiled, program.Operand_Index(u32(instruction.operands_start)+2))
	if !index_ok || !key_ok || !rhs_ok || index_operand.kind != .Text || key_operand.kind != .Text || rhs_operand.kind != .Instruction do return
	index_text, index_ok = program.operand_text(storage.compiled, index_operand)
	key, key_ok = program.operand_text(storage.compiled, key_operand)
	_, rhs_ok = program.program_instruction(storage.compiled, rhs_operand.instruction)
	rhs = rhs_operand.instruction
	ok = index_ok && key_ok && rhs_ok
	return
}

@(private)
static_field_index_field_update_operands :: proc(storage: ^evaluator_storage, instruction: program.Instruction) -> (base_key, index_text, key: string, rhs: program.Instruction_Index, ok: bool) {
	if instruction.opcode != .Static_Field_Index_Field_Update || instruction.operands_count != 4 do return
	base_operand, base_ok := program.program_operand(storage.compiled, instruction.operands_start)
	index_operand, index_ok := program.program_operand(storage.compiled, program.Operand_Index(u32(instruction.operands_start)+1))
	key_operand, key_ok := program.program_operand(storage.compiled, program.Operand_Index(u32(instruction.operands_start)+2))
	rhs_operand, rhs_ok := program.program_operand(storage.compiled, program.Operand_Index(u32(instruction.operands_start)+3))
	if !base_ok || !index_ok || !key_ok || !rhs_ok || base_operand.kind != .Text || index_operand.kind != .Text || key_operand.kind != .Text || rhs_operand.kind != .Instruction do return
	base_key, base_ok = program.operand_text(storage.compiled, base_operand)
	index_text, index_ok = program.operand_text(storage.compiled, index_operand)
	key, key_ok = program.operand_text(storage.compiled, key_operand)
	_, rhs_ok = program.program_instruction(storage.compiled, rhs_operand.instruction)
	rhs = rhs_operand.instruction
	ok = base_ok && index_ok && key_ok && rhs_ok
	return
}
...
@(private)
iterator_update_delete_at :: proc(storage: ^evaluator_storage, frame: ^eval_frame, index: int) -> (runtime.Allocator_Error, bool) {
	kind := value.kind_of(&frame.input)
	if kind == .Array {
		length, ok := value.array_length(&frame.input)
		if !ok || index < 0 || index >= length do return nil, false
		replacement, array_error := value.array_value(storage.allocator)
		if value.array_error_kind(&array_error) != .None {
			error_kind := value.array_error_kind(&array_error)
			cleanup := value.destroy_array_error(&array_error)
		...
		for cursor in 0..<length {
			if cursor == index do continue
			item, item_ok := value.array_element_copy(&frame.input, cursor)
			if !item_ok { _ = value.destroy_value(&replacement); return nil, false }
			_, append_error := value.array_append_take(&replacement, &item)
			if value.array_error_kind(&append_error) != .None {
				_ = value.destroy_value(&item)
				_ = value.destroy_value(&replacement)
				error_kind := value.array_error_kind(&append_error)
				cleanup := value.destroy_array_error(&append_error)
				if cleanup != nil do return cleanup, false
				return .Out_Of_Memory if error_kind == .Out_Of_Memory else nil, false
			}
		}
		_ = value.destroy_value(&frame.input)
		frame.input = replacement
		return nil, true
	}
	if kind == .Object {
		key, entry, key_ok := value.object_entry_copy(&frame.input, index)
		if !key_ok do return nil, false
		_ = value.destroy_value(&entry)
		key_text, text_ok := value.string_borrowed(&key)
		if !text_ok { _ = value.destroy_value(&key); return nil, false }
		removed_key, removed, found, delete_error := value.object_delete_take(&frame.input, key_text)
		_ = value.destroy_value(&key)
		_ = value.destroy_value(&removed_key)
		_ = value.destroy_value(&removed)
		if value.object_error_kind(&delete_error) != .None {
			error_kind := value.object_error_kind(&delete_error)
			cleanup := value.destroy_object_error(&delete_error)
			if cleanup != nil do return cleanup, false
			return .Out_Of_Memory if error_kind == .Out_Of_Memory else nil, false
		}
		return nil, found
...
resumed_composite_instruction_valid :: proc(
	...
	case .Iterator_Update_Active, .Iterator_Update_Child_Active, .Iterator_Update_Delete_Active:
	...
	case .Static_Index_Field_Update_Child_Active, .Static_Index_Field_Update_Empty:
	...
	case .Dynamic_Index_Assign_Key_Active, .Dynamic_Index_Assign_RHS_Start, .Dynamic_Index_Assign_RHS_Active, .Dynamic_Index_Assign_Apply:
		if frame.mode != .Normal || instruction.opcode != .Dynamic_Index_Assign do return false
	...
	if frame.phase == .Unary_Active || frame.phase == .First_Empty || frame.phase == .Last_Result || frame.phase == .Last_Empty || frame.phase == .Add_Active || frame.phase == .Add_Result || frame.phase == .Add_Empty do expected_operand_count = 1
	...
	else if frame.phase == .Iterator_Update_Active || frame.phase == .Iterator_Update_Child_Active || frame.phase == .Iterator_Update_Delete_Active {
		...
		expected_operand_count = 3
	} else if frame.phase == .Static_Index_Field_Update_Child_Active || frame.phase == .Static_Index_Field_Update_Empty {
		expected_operand_count = 3
		...
		expected_operand_count = 2
	} else if frame.phase == .Dynamic_Index_Assign_Key_Active || frame.phase == .Dynamic_Index_Assign_RHS_Start || frame.phase == .Dynamic_Index_Assign_RHS_Active || frame.phase == .Dynamic_Index_Assign_Apply {
		expected_operand_count = 3
	}
...
@(private)
path_array :: proc(allocator: runtime.Allocator) -> (value.Value, bool) {
	result, err := value.array_value(allocator)
	if value.array_error_kind(&err) != .None { _ = value.destroy_array_error(&err); return {}, false }
	return result, true
}

@(private)
path_append_take :: proc(path: ^value.Value, component: ^value.Value) -> bool {
	_, err := value.array_append_take(path, component)
	if value.array_error_kind(&err) != .None { _ = value.destroy_array_error(&err); _ = value.destroy_value(component); return false }
	return true
}

@(private)
static_filter_path :: proc(storage: ^evaluator_storage, index: program.Instruction_Index) -> (value.Value, bool) {
	instruction, ok := program.program_instruction(storage.compiled, index)
	if !ok do return {}, false
	if instruction.opcode == .Parenthesized { child, child_ok := child_instruction(storage, instruction, 0); if !child_ok do return {}, false; return static_filter_path(storage, child) }
	result, result_ok := path_array(storage.allocator); if !result_ok do return {}, false
	if instruction.opcode == .Identity && !instruction.has_literal && instruction.operands_count == 0 do return result, true
	if instruction.opcode == .Field {
		name, name_ok := field_text(storage, instruction); if !name_ok { _ = value.destroy_value(&result); return {}, false }
		if instruction.operands_count == 2 { child, child_ok := child_instruction(storage, instruction, 0); if !child_ok { _ = value.destroy_value(&result); return {}, false }; prefix, prefix_ok := static_filter_path(storage, child); if !prefix_ok { _ = value.destroy_value(&result); return {}, false }; _ = value.destroy_value(&result); result = prefix }
		component, err := value.string_value(name, storage.allocator); if value.constructor_error_kind(&err) != .None { _ = value.destroy_constructor_error(&err); _ = value.destroy_value(&result); return {}, false }
		if !path_append_take(&result, &component) { _ = value.destroy_value(&result); return {}, false }; return result, true
	}
	if instruction.opcode == .Index && instruction.operands_count == 2 {
		child, child_ok := child_instruction(storage, instruction, 0); if !child_ok { _ = value.destroy_value(&result); return {}, false }; prefix, prefix_ok := static_filter_path(storage, child); if !prefix_ok { _ = value.destroy_value(&result); return {}, false }; _ = value.destroy_value(&result); result = prefix
		operand, operand_ok := program.program_operand(storage.compiled, program.Operand_Index(u32(instruction.operands_start)+1)); text, text_ok := program.operand_text(storage.compiled, operand)
		if !operand_ok || !text_ok || operand.kind != .Text { _ = value.destroy_value(&result); return {}, false }; component, err := value.literal_number_value(text, storage.allocator); if value.constructor_error_kind(&err) != .None { _ = value.destroy_constructor_error(&err); _ = value.destroy_value(&result); return {}, false }; if !path_append_take(&result, &component) { _ = value.destroy_value(&result); return {}, false }; return result, true
	}
...
// so this remains an explicit AST contract rather than accepting arbitrary
// dynamic filters and guessing their paths.
dynamic_path_prefix :: proc(storage: ^evaluator_storage, index: program.Instruction_Index) -> (value.Value, bool, bool) {
	...
	if instruction.opcode == .Field {
		name, name_ok := field_text(storage, instruction)
		if !name_ok { _ = value.destroy_value(&result); return {}, false, false }
		if name == "" {
			if instruction.operands_count != 2 {
				_ = value.destroy_value(&result)
				return {}, false, false
			...
			child, child_ok := child_instruction(storage, instruction, 0)
			if !child_ok { _ = value.destroy_value(&result); return {}, false, false }
			prefix, _, prefix_ok := dynamic_path_prefix(storage, child)
			if !prefix_ok { _ = value.destroy_value(&result); return {}, false, false }
			_ = value.destroy_value(&result)
			return prefix, true, true
		}
		if instruction.operands_count == 2 {
			...
			if !prefix_ok || wildcard { _ = value.destroy_value(&prefix); _ = value.destroy_value(&result); return {}, false, false }
			_ = value.destroy_value(&result)
			result = prefix
		}
		component, err := value.string_value(name, storage.allocator)
		if value.constructor_error_kind(&err) != .None { _ = value.destroy_constructor_error(&err); _ = value.destroy_value(&result); return {}, false, false }
		if !path_append_take(&result, &component) { _ = value.destroy_value(&result); return {}, false, false }
		return result, false, true
	}
	if instruction.opcode == .Index && instruction.operands_count == 2 {
		child, child_ok := child_instruction(storage, instruction, 0)
		...
		operand, operand_ok := program.program_operand(storage.compiled, program.Operand_Index(u32(instruction.operands_start)+1))
		text, text_ok := program.operand_text(storage.compiled, operand)
		if !operand_ok || !text_ok || operand.kind != .Text { _ = value.destroy_value(&result); return {}, false, false }
		component, err := value.literal_number_value(text, storage.allocator)
		if value.constructor_error_kind(&err) != .None { _ = value.destroy_constructor_error(&err); _ = value.destroy_value(&result); return {}, false, false }
		if !path_append_take(&result, &component) { _ = value.destroy_value(&result); return {}, false, false }
		return result, false, true
	}
...
dynamic_path_results :: proc(input, prefix: ^value.Value, allocator: runtime.Allocator) -> (value.Value, Runtime_Error_Kind, bool) {
	...
	kind := value.kind_of(&container)
	length: int
	length_ok := false
	if kind == .Array { length, length_ok = value.array_length(&container) } else if kind == .Object { length, length_ok = value.object_length(&container) }
	...
	for offset in 0..<length {
		...
		if kind == .Array {
			...
			key, _, entry_ok := value.object_entry_copy(&container, offset)
			if !entry_ok { _ = value.destroy_value(&path); _ = value.destroy_value(&container); _ = value.destroy_value(&output); return {}, .None, false }
			component = key
		}
...
literal_path_value :: proc(storage: ^evaluator_storage, index: program.Instruction_Index) -> (value.Value, bool) {
	...
	if instruction.opcode == .Parenthesized { child, child_ok := child_instruction(storage, instruction, 0); if !child_ok do return {}, false; return literal_path_value(storage, child) }
	if instruction.opcode != .Array do return {}, false
	result, result_ok := path_array(storage.allocator); if !result_ok do return {}, false
	append_item :: proc(storage: ^evaluator_storage, item_index: program.Instruction_Index, result: ^value.Value) -> bool {
	...
	for offset in 0..<instruction.operands_count {
		operand, operand_ok := program.program_operand(storage.compiled, program.Operand_Index(u32(instruction.operands_start)+u32(offset)))
		if !operand_ok || operand.kind != .Instruction { _ = value.destroy_value(&result); return {}, false }
		if !append_item(storage, operand.instruction, &result) { _ = value.destroy_value(&result); return {}, false }
	}
	return result, true
}
...
lookup_path :: proc(input, path: ^value.Value) -> (value.Value, Runtime_Error_Kind, bool) {
	...
	length, length_ok := value.array_length(path); if !length_ok { _ = value.destroy_value(&current); return {}, .None, false }
	for offset in 0..<length {
		...
		_ = value.destroy_value(&component); _ = value.destroy_value(&current); current = next
	}
...
@(private)
dynamic_index_assign_runtime_error :: proc(input, key: ^value.Value, span: program.Source_Span, allocator: runtime.Allocator) -> (Runtime_Error, bool) {
	input_kind := value.kind_of(input)
	key_kind := value.kind_of(key)
	valid := (key_kind == .String && (input_kind == .Object || input_kind == .Null)) ||
		(key_kind == .Number && (input_kind == .Array || input_kind == .Null))
	if valid do return {}, false
	message: string
	if input_kind == .Array || input_kind == .String || input_kind == .Null {
		message, _ = strings.concatenate([]string{"Array/string slice indices must be integers"}, allocator)
	} else {
		message = strings.concatenate([]string{"Cannot index ", runtime_value_kind_name(input_kind), " with ", runtime_value_kind_name(key_kind)}, allocator)
	}
	return Runtime_Error{kind=.User_Error, input_kind=input_kind, span=span, key=message}, true
}
...
paths_result :: proc(input: ^value.Value, allocator: runtime.Allocator) -> (value.Value, bool) {
	...
	walk :: proc(v, prefix, out: ^value.Value, depth: int) -> bool {
		if depth > 256 do return true; kind := value.kind_of(v); if kind != .Array && kind != .Object do return true; count := 0; if kind == .Array { count, _ = value.array_length(v) } else { count, _ = value.object_length(v) }
		for i in 0..<count { component: value.Value; child: value.Value; if kind == .Array { component = value.number_value(f64(i)); child, _ = value.array_element_copy(v, i) } else { key, item, got := value.object_entry_copy(v, i); if !got do return false; component, child = key, item }; child_path := value.clone_value(prefix); if !path_append_take(&child_path, &component) { _ = value.destroy_value(&child); return false }; one := value.clone_value(&child_path); if !path_append_take(out, &one) { _ = value.destroy_value(&child); _ = value.destroy_value(&child_path); return false }; good := walk(&child, &child_path, out, depth+1); _ = value.destroy_value(&child); _ = value.destroy_value(&child_path); if !good do return false }; return true
	}
...
destroy_frames_to :: proc(storage: ^evaluator_storage, target_count: int) -> runtime.Allocator_Error {
	for storage.frame_count > target_count {
		...
		if free_error != nil do return free_error
		free_error = value.destroy_value(&storage.frames[index].dynamic_index_assign_keys)
		if free_error != nil do return free_error
		free_error = value.destroy_value(&storage.frames[index].dynamic_index_assign_rhs)
		if free_error != nil do return free_error
...
notify_exhausted :: proc(storage: ^evaluator_storage, parent: int) -> bool {
	...
	case .Static_Field_Index_Update_Empty:
		return false
	case .Static_Index_Field_Update_Child_Active:
		frame.phase = .Static_Index_Field_Update_Empty
	case .Static_Index_Field_Update_Empty:
		return false
	case .Static_Field_Index_Field_Update_Child_Active:
	...
	case .Path_Assign_Apply:
		return false
	case .Dynamic_Index_Assign_Key_Active:
		frame.phase = .Dynamic_Index_Assign_RHS_Start
	case .Dynamic_Index_Assign_RHS_Active:
		frame.phase = .Dynamic_Index_Assign_Apply
	case .Dynamic_Index_Assign_Apply:
		return false
...
propagate_output :: proc(
	...
	for {
		...
		case .In_Child_Active:
			matched := value.values_equal(owned, &frame.input)
			_ = value.destroy_value(owned)
		...
		case .Field_Result_Active:
			current = parent
		case .Static_Field_Update_Child_Active:
			if frame.static_field_update_seen {
				_ = value.destroy_value(owned)
				return {}, false
			}
			frame.static_field_update_seen = true
			...
			duplicate, displaced, set_error := value.object_set_take(&frame.input, &key, owned)
			if value.object_error_kind(&set_error) != .None {
				_ = value.destroy_value(&key)
				_ = value.destroy_value(owned)
				_ = value.destroy_value(&duplicate)
		...
		case .Static_Field_Index_Update_Child_Active:
			...
			return propagate_output(storage, parent, &output)
		case .Static_Index_Field_Update_Child_Active:
			if frame.static_index_field_update_seen { _ = value.destroy_value(owned); return {}, false }
			frame.static_index_field_update_seen = true
			capacity_error := prepare_output(storage, parent)
			if capacity_error != nil { storage.pending_value = value.take_value(owned); return resource_step(capacity_error), true }
			frame = &storage.frames[parent]
			_, key_text, _, operands_ok := static_index_field_update_operands(storage, instruction)
			if !operands_ok { return begin_terminal_misuse_owned(storage, .Malformed_Program, owned), true }
			selected: value.Value
			if frame.static_index_field_in_range { selected, _ = value.array_element_copy(&frame.input, frame.static_index_field) } else { selected = value.null_value() }
			if value.kind_of(&selected) == .Null {
				empty_object, object_error := value.object_value(storage.allocator)
				if value.object_error_kind(&object_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_value(owned); _ = value.destroy_object_error(&object_error); return resource_step(.Out_Of_Memory), true }
				_ = value.destroy_value(&selected); selected = empty_object
			}
			key, key_ok := existing_object_key_copy(&selected, key_text)
			if !key_ok { key_error: value.Constructor_Error; key, key_error = value.string_value(key_text, storage.allocator); if value.constructor_error_kind(&key_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_value(owned); _ = value.destroy_constructor_error(&key_error); return resource_step(.Out_Of_Memory), true } }
			duplicate, displaced, object_error := value.object_set_take(&selected, &key, owned)
			_ = value.destroy_value(&key); _ = value.destroy_value(&duplicate); _ = value.destroy_value(&displaced)
			if value.object_error_kind(&object_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_object_error(&object_error); return begin_terminal_misuse(storage, .Malformed_Program), true }
			displaced_array, array_error := value.array_set_take(&frame.input, frame.static_index_field, &selected)
			_ = value.destroy_value(&displaced_array)
			if value.array_error_kind(&array_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_array_error(&array_error); return begin_terminal_misuse(storage, .Malformed_Program), true }
			free_error := destroy_frames_to(storage, parent+1); if free_error != nil { return resource_step(free_error), true }
			frame = &storage.frames[parent]; frame.phase = .Complete
			output := value.take_value(&frame.input)
			return propagate_output(storage, parent, &output)
		case .Static_Field_Index_Field_Update_Child_Active:
			if frame.static_field_index_field_update_seen { _ = value.destroy_value(owned); return {}, false }
			frame.static_field_index_field_update_seen = true
			capacity_error := prepare_output(storage, parent)
			if capacity_error != nil { storage.pending_value = value.take_value(owned); return resource_step(capacity_error), true }
			frame = &storage.frames[parent]
			base_key, _, key_text, _, operands_ok := static_field_index_field_update_operands(storage, instruction)
			if !operands_ok { return begin_terminal_misuse_owned(storage, .Malformed_Program, owned), true }
			base_array: value.Value
			base_array_error: value.Array_Operation_Error
			if frame.static_field_index_field_missing {
				base_array, base_array_error = value.array_value(storage.allocator)
				if value.array_error_kind(&base_array_error) != .None { _ = value.destroy_value(owned); _ = value.destroy_array_error(&base_array_error); return resource_step(.Out_Of_Memory), true }
			} else {
				found: bool
				base_array, found = value.object_get_copy(&frame.input, base_key)
				if !found || value.kind_of(&base_array) != .Array { _ = value.destroy_value(&base_array); _ = value.destroy_value(owned); return begin_terminal_misuse(storage, .Malformed_Program), true }
			}
			selected: value.Value
			if frame.static_field_index_field_in_range { selected, _ = value.array_element_copy(&base_array, frame.static_field_index_field) } else { selected = value.null_value() }
			if value.kind_of(&selected) == .Null {
				empty_object, object_error := value.object_value(storage.allocator)
				if value.object_error_kind(&object_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_value(&base_array); _ = value.destroy_object_error(&object_error); return resource_step(.Out_Of_Memory), true }
				_ = value.destroy_value(&selected); selected = empty_object
			}
			key, key_ok := existing_object_key_copy(&selected, key_text)
			if !key_ok { key_error: value.Constructor_Error; key, key_error = value.string_value(key_text, storage.allocator); if value.constructor_error_kind(&key_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_value(&base_array); _ = value.destroy_value(owned); _ = value.destroy_constructor_error(&key_error); return resource_step(.Out_Of_Memory), true } }
			duplicate, displaced, object_error := value.object_set_take(&selected, &key, owned)
			_ = value.destroy_value(&key); _ = value.destroy_value(&duplicate); _ = value.destroy_value(&displaced)
			if value.object_error_kind(&object_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_value(&base_array); _ = value.destroy_object_error(&object_error); return begin_terminal_misuse(storage, .Malformed_Program), true }
		displaced_item, array_error := value.array_set_take(&base_array, frame.static_field_index_field, &selected); _ = value.destroy_value(&displaced_item)
			if value.array_error_kind(&array_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_value(&base_array); _ = value.destroy_array_error(&array_error); return begin_terminal_misuse(storage, .Malformed_Program), true }
			field_key, field_key_ok := existing_object_key_copy(&frame.input, base_key)
			if !field_key_ok { key_error: value.Constructor_Error; field_key, key_error = value.string_value(base_key, storage.allocator); if value.constructor_error_kind(&key_error) != .None { _ = value.destroy_value(&base_array); _ = value.destroy_constructor_error(&key_error); return resource_step(.Out_Of_Memory), true } }
			duplicate_field, displaced_field, root_error := value.object_set_take(&frame.input, &field_key, &base_array)
			_ = value.destroy_value(&duplicate_field); _ = value.destroy_value(&displaced_field)
			if value.object_error_kind(&root_error) != .None { _ = value.destroy_value(&field_key); _ = value.destroy_value(&base_array); _ = value.destroy_object_error(&root_error); return begin_terminal_misuse(storage, .Malformed_Program), true }
			free_error := destroy_frames_to(storage, parent+1); if free_error != nil { return resource_step(free_error), true }
			frame = &storage.frames[parent]; frame.phase = .Complete
			output := value.take_value(&frame.input)
			return propagate_output(storage, parent, &output)
	...
	case .Path_Assign_RHS_Active:
		...
		return {}, false
	case .Dynamic_Index_Assign_Key_Active:
		duplicate, append_error := value.array_append_take(&frame.dynamic_index_assign_keys, owned)
		_ = value.destroy_value(&duplicate)
		if value.array_error_kind(&append_error) != .None { _ = value.destroy_value(owned); _ = value.destroy_array_error(&append_error); return resource_step(.Out_Of_Memory), true }
		return {}, false
	case .Dynamic_Index_Assign_RHS_Active:
		duplicate, append_error := value.array_append_take(&frame.dynamic_index_assign_rhs, owned)
		_ = value.destroy_value(&duplicate)
		if value.array_error_kind(&append_error) != .None { _ = value.destroy_value(owned); _ = value.destroy_array_error(&append_error); return resource_step(.Out_Of_Memory), true }
		return {}, false
	case .Index_Result_Active:
	...
	case .Slice_Result_Active:
		...
		case .Iterator_Update_Child_Active:
			...
			frame.iterator_update_seen = true
			update_index := frame.iterator_cursor - 1
			if value.kind_of(&frame.input) == .Array {
				displaced, set_error := value.array_set_take(&frame.input, update_index, owned)
				_ = value.destroy_value(&displaced)
				if value.array_error_kind(&set_error) != .None {
					_ = value.destroy_value(owned)
					_ = value.destroy_array_error(&set_error)
					return begin_terminal_misuse(storage, .Malformed_Program), true
				}
			} else if value.kind_of(&frame.input) == .Object {
				key, entry, key_ok := value.object_entry_copy(&frame.input, update_index)
				if !key_ok { _ = value.destroy_value(owned); return begin_terminal_misuse(storage, .Malformed_Program), true }
				_ = value.destroy_value(&entry)
				duplicate, displaced, set_error := value.object_set_take(&frame.input, &key, owned)
				_ = value.destroy_value(&duplicate)
				_ = value.destroy_value(&displaced)
...
step_evaluator :: proc(evaluator: ^Evaluator) -> Step_Result {
	...
	for {
		...
		case .Enter:
			...
			if instruction.opcode == .Dynamic_Index_Assign {
				if instruction.operands_count != 3 || !capture_composite_instruction(storage, frame, instruction) do return begin_terminal_misuse(storage, .Malformed_Program)
				base_child, base_ok := child_instruction(storage, instruction, 0)
				key_child, key_ok := child_instruction(storage, instruction, 1)
				if !base_ok || !key_ok { return begin_terminal_misuse(storage, .Malformed_Program) }
				base_instruction, base_instruction_ok := program.program_instruction(storage.compiled, base_child)
				if !base_instruction_ok || base_instruction.opcode != .Identity { return begin_terminal_misuse(storage, .Unsupported_Opcode) }
				keys, keys_error := value.array_value(storage.allocator)
				if value.array_error_kind(&keys_error) != .None { _ = value.destroy_array_error(&keys_error); return resource_step(.Out_Of_Memory) }
				frame.dynamic_index_assign_keys = value.take_value(&keys)
				rhs, rhs_error := value.array_value(storage.allocator)
				if value.array_error_kind(&rhs_error) != .None { _ = value.destroy_array_error(&rhs_error); _ = value.destroy_value(&frame.dynamic_index_assign_keys); return resource_step(.Out_Of_Memory) }
				frame.dynamic_index_assign_rhs = value.take_value(&rhs)
				input_copy := value.clone_value(&frame.input)
				if value.kind_of(&input_copy) == .Invalid || !push_frame(storage, key_child, index, &input_copy) { _ = value.destroy_value(&frame.dynamic_index_assign_keys); _ = value.destroy_value(&frame.dynamic_index_assign_rhs); return begin_terminal_misuse_owned(storage, .Malformed_Program, &input_copy) }
				frame.phase = .Dynamic_Index_Assign_Key_Active
				continue
			}
			...
			if instruction.opcode == .Paths {
				if instruction.operands_count != 0 do return begin_terminal_misuse(storage, .Malformed_Program)
				results, results_ok := paths_result(&frame.input, storage.allocator); if !results_ok do return resource_step(.Out_Of_Memory)
				frame.paths_results = value.take_value(&results); frame.paths_cursor = 0; frame.phase = .Paths_Active; continue
			}
			...
			case .Static_Field_Optional_Identity:
				...
				if ready do return result
			case .Static_Field_Update:
				key_text, rhs_instruction, operands_ok := static_field_update_operands(storage, instruction)
				...
				if input_kind == .Object {
					found: bool
					replacement, found = value.object_get_copy(&frame.input, key_text)
					if !found do replacement = value.null_value()
				} else {
			...
			case .Static_Index_Field_Update:
				index_text, key_text, rhs_instruction, operands_ok := static_index_field_update_operands(storage, instruction)
				if !operands_ok || !capture_composite_instruction(storage, frame, instruction) do return begin_terminal_misuse(storage, .Malformed_Program)
				index_float, index_ok := strconv.parse_f64(index_text)
				if !index_ok || math.is_nan(index_float) || math.is_inf(index_float) || index_float < 0 || index_float > f64(max(int)) { return begin_terminal_misuse(storage, .Unsupported_Opcode) }
				input_kind := value.kind_of(&frame.input)
				if input_kind != .Array && input_kind != .Null {
					message := "Cannot index object with number"
					switch input_kind { case .Boolean: message = "Cannot index boolean with number"; case .Number: message = "Cannot index number with number"; case .String: message = "Cannot index string with number"; case .Object: message = "Cannot index object with number"; case .Array, .Null, .Invalid: }
					result, ready := raise_runtime(storage, index, Runtime_Error{kind=.User_Error, input_kind=input_kind, span=instruction.span, key=message}); if ready do return result
					continue
				}
				frame.static_index_field_root_null = input_kind == .Null
				if input_kind == .Null {
					empty_array, array_error := value.array_value(storage.allocator)
				...
				frame.static_index_field = int(index_float)
				length, length_ok := value.array_length(&frame.input); if !length_ok do return begin_terminal_misuse(storage, .Malformed_Program)
				frame.static_index_field_in_range = frame.static_index_field < length
				replacement := value.null_value()
				if frame.static_index_field_in_range { replacement, _ = value.array_element_copy(&frame.input, frame.static_index_field) }
				selected_kind := value.kind_of(&replacement)
				if selected_kind == .Object {
					nested, found := value.object_get_copy(&replacement, key_text)
					if found {
						_ = value.destroy_value(&replacement)
						replacement = nested
					} else {
				...
				if selected_kind != .Null && selected_kind != .Object {
					kind := selected_kind; _ = value.destroy_value(&replacement)
					result, ready := raise_runtime(storage, index, Runtime_Error{kind=.Cannot_Index_With_String, input_kind=kind, span=instruction.span, key=key_text}); if ready do return result
					continue
				}
				if storage.frame_count == len(storage.frames) { capacity_error := grow_frames(storage); if capacity_error != nil { _ = value.destroy_value(&replacement); return resource_step(capacity_error) }; frame = &storage.frames[index] }
			if !push_frame(storage, rhs_instruction, index, &replacement) { _ = value.destroy_value(&replacement); return begin_terminal_misuse(storage, .Malformed_Program) }
			storage.frames[index].static_index_field_update_seen = false
			storage.frames[index].phase = .Static_Index_Field_Update_Child_Active
			case .Static_Field_Index_Field_Update:
				base_key, index_text, key_text, rhs_instruction, operands_ok := static_field_index_field_update_operands(storage, instruction)
				if !operands_ok || !capture_composite_instruction(storage, frame, instruction) do return begin_terminal_misuse(storage, .Malformed_Program)
				index_float, index_ok := strconv.parse_f64(index_text)
				if !index_ok || math.is_nan(index_float) || math.is_inf(index_float) || index_float < 0 || index_float > f64(max(int)) { return begin_terminal_misuse(storage, .Unsupported_Opcode) }
				input_kind := value.kind_of(&frame.input)
				if input_kind != .Object && input_kind != .Null {
					result, ready := raise_runtime(storage, index, Runtime_Error{kind=.Cannot_Index_With_String, input_kind=input_kind, span=instruction.span, key=base_key}); if ready do return result
					continue
				}
				frame.static_field_index_field_root_null = input_kind == .Null
				if input_kind == .Null {
					empty_object, object_error := value.object_value(storage.allocator)
					if value.object_error_kind(&object_error) != .None { _ = value.destroy_object_error(&object_error); return resource_step(.Out_Of_Memory) }
					_ = value.destroy_value(&frame.input); frame.input = empty_object
				}
				base_value, found := value.object_get_copy(&frame.input, base_key)
				frame.static_field_index_field_missing = !found || value.kind_of(&base_value) == .Null
				if !found || value.kind_of(&base_value) == .Null {
					_ = value.destroy_value(&base_value)
					array_error: value.Array_Operation_Error
					base_value, array_error = value.array_value(storage.allocator)
					if value.array_error_kind(&array_error) != .None { _ = value.destroy_array_error(&array_error); return resource_step(.Out_Of_Memory) }
				} else if value.kind_of(&base_value) != .Array {
					kind := value.kind_of(&base_value); _ = value.destroy_value(&base_value)
					message := "Cannot index object with number"
					switch kind { case .Boolean: message = "Cannot index boolean with number"; case .Number: message = "Cannot index number with number"; case .String: message = "Cannot index string with number"; case .Object: message = "Cannot index object with number"; case .Array, .Null, .Invalid: }
					result, ready := raise_runtime(storage, index, Runtime_Error{kind=.User_Error, input_kind=kind, span=instruction.span, key=message}); if ready do return result
					continue
				}
				frame.static_field_index_field = int(index_float)
				length, length_ok := value.array_length(&base_value); if !length_ok { _ = value.destroy_value(&base_value); return begin_terminal_misuse(storage, .Malformed_Program) }
				frame.static_field_index_field_in_range = frame.static_field_index_field < length
				replacement := value.null_value()
				if frame.static_field_index_field_in_range {
					selected, selected_ok := value.array_element_copy(&base_value, frame.static_field_index_field)
					if !selected_ok { _ = value.destroy_value(&base_value); return begin_terminal_misuse(storage, .Malformed_Program) }
					selected_kind := value.kind_of(&selected)
					if selected_kind == .Object {
						replacement, found = value.object_get_copy(&selected, key_text); if !found do replacement = value.null_value()
						_ = value.destroy_value(&selected)
					} else if selected_kind == .Null {
						_ = value.destroy_value(&selected)
					} else {
						_ = value.destroy_value(&selected); _ = value.destroy_value(&base_value)
						result, ready := raise_runtime(storage, index, Runtime_Error{kind=.Cannot_Index_With_String, input_kind=selected_kind, span=instruction.span, key=key_text}); if ready do return result
						continue
					}
				}
				_ = value.destroy_value(&base_value)
				if storage.frame_count == len(storage.frames) { capacity_error := grow_frames(storage); if capacity_error != nil { _ = value.destroy_value(&replacement); return resource_step(capacity_error) }; frame = &storage.frames[index] }
				if !push_frame(storage, rhs_instruction, index, &replacement) { _ = value.destroy_value(&replacement); return begin_terminal_misuse(storage, .Malformed_Program) }
				storage.frames[index].static_field_index_field_update_seen = false
				storage.frames[index].phase = .Static_Field_Index_Field_Update_Child_Active
			case .Static_Field_Index_Update:
			...
			case .Static_Field_Add_Number:
				...
				if !found do old = value.null_value()
		...
		case .Static_Field_Index_Update_Empty:
			...
			if ready do return result
		case .Static_Index_Field_Update_Empty:
			index_text, key_text, _, operands_ok := static_index_field_update_operands(storage, instruction)
			if !operands_ok do return begin_terminal_misuse(storage, .Malformed_Program)
			_ = index_text
			if frame.static_index_field_root_null {
				_ = value.destroy_value(&frame.input)
				frame.input = value.null_value()
			} else if frame.static_index_field_in_range {
				selected, selected_ok := value.array_element_copy(&frame.input, frame.static_index_field)
				if !selected_ok do return begin_terminal_misuse(storage, .Malformed_Program)
				if value.kind_of(&selected) == .Object {
					key, found := existing_object_key_copy(&selected, key_text)
					if found { _, removed, _, delete_error := value.object_delete_take(&selected, key_text); _ = value.destroy_value(&key); _ = value.destroy_value(&removed); if value.object_error_kind(&delete_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_object_error(&delete_error); return begin_terminal_misuse(storage, .Malformed_Program) } } else { _ = value.destroy_value(&key) }
					displaced, array_error := value.array_set_take(&frame.input, frame.static_index_field, &selected); _ = value.destroy_value(&displaced); if value.array_error_kind(&array_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_array_error(&array_error); return begin_terminal_misuse(storage, .Malformed_Program) }
				} else if value.kind_of(&selected) == .Null {
					// Keep the null element owned for the array replacement below.
				} else {
					selected_kind := value.kind_of(&selected)
					_ = value.destroy_value(&selected)
					result, ready := raise_runtime(storage, index, Runtime_Error{kind=.Cannot_Index_With_String, input_kind=selected_kind, span=instruction.span, key=key_text})
					if ready do return result
					continue
				}
			}
		...
		case .Static_Field_Index_Field_Update_Empty:
			base_key, _, key_text, _, operands_ok := static_field_index_field_update_operands(storage, instruction)
			if !operands_ok do return begin_terminal_misuse(storage, .Malformed_Program)
			if frame.static_field_index_field_root_null {
				_ = value.destroy_value(&frame.input)
				frame.input = value.null_value()
			} else if !frame.static_field_index_field_missing && frame.static_field_index_field_in_range {
				base_array, found := value.object_get_copy(&frame.input, base_key)
				if !found || value.kind_of(&base_array) != .Array { _ = value.destroy_value(&base_array); return begin_terminal_misuse(storage, .Malformed_Program) }
				selected, selected_ok := value.array_element_copy(&base_array, frame.static_field_index_field)
				if !selected_ok { _ = value.destroy_value(&base_array); return begin_terminal_misuse(storage, .Malformed_Program) }
				if value.kind_of(&selected) == .Object {
					key, found_key := existing_object_key_copy(&selected, key_text)
					if found_key { _, removed, _, delete_error := value.object_delete_take(&selected, key_text); _ = value.destroy_value(&key); _ = value.destroy_value(&removed); if value.object_error_kind(&delete_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_value(&base_array); _ = value.destroy_object_error(&delete_error); return begin_terminal_misuse(storage, .Malformed_Program) } } else { _ = value.destroy_value(&key) }
				} else if value.kind_of(&selected) == .Null {
					// Keep null selected for array_set_take below.
				} else {
					selected_kind := value.kind_of(&selected); _ = value.destroy_value(&selected); _ = value.destroy_value(&base_array)
					result, ready := raise_runtime(storage, index, Runtime_Error{kind=.Cannot_Index_With_String, input_kind=selected_kind, span=instruction.span, key=key_text}); if ready do return result
					continue
				}
				displaced, array_error := value.array_set_take(&base_array, frame.static_field_index_field, &selected); _ = value.destroy_value(&displaced)
				if value.array_error_kind(&array_error) != .None { _ = value.destroy_value(&selected); _ = value.destroy_value(&base_array); _ = value.destroy_array_error(&array_error); return begin_terminal_misuse(storage, .Malformed_Program) }
				field_key, field_key_ok := existing_object_key_copy(&frame.input, base_key)
				if !field_key_ok { _ = value.destroy_value(&base_array); return begin_terminal_misuse(storage, .Malformed_Program) }
				duplicate, displaced_field, root_error := value.object_set_take(&frame.input, &field_key, &base_array); _ = value.destroy_value(&duplicate); _ = value.destroy_value(&displaced_field)
		...
		case .Dynamic_Index_Assign_RHS_Start:
			results, results_error := value.array_value(storage.allocator)
			if value.array_error_kind(&results_error) != .None { _ = value.destroy_array_error(&results_error); return resource_step(.Out_Of_Memory) }
			_ = value.destroy_value(&frame.dynamic_index_assign_rhs)
			frame.dynamic_index_assign_rhs = value.take_value(&results)
			frame.dynamic_index_assign_rhs_cursor = 0
			rhs_child, rhs_ok := child_instruction(storage, instruction, 2)
			input_copy := value.clone_value(&frame.input)
			if !rhs_ok || value.kind_of(&input_copy) == .Invalid || !push_frame(storage, rhs_child, index, &input_copy) { _ = value.destroy_value(&input_copy); _ = value.destroy_value(&frame.dynamic_index_assign_rhs); return begin_terminal_misuse(storage, .Malformed_Program) }
			frame.phase = .Dynamic_Index_Assign_RHS_Active
		case .Dynamic_Index_Assign_Apply:
			key_length, key_length_ok := value.array_length(&frame.dynamic_index_assign_keys)
			rhs_length, rhs_length_ok := value.array_length(&frame.dynamic_index_assign_rhs)
			if !key_length_ok || !rhs_length_ok { return begin_terminal_misuse(storage, .Malformed_Program) }
			if key_length == 0 || frame.dynamic_index_assign_rhs_cursor >= rhs_length { frame.phase = .Complete; continue }
			template, template_ok := value.array_element_copy(&frame.dynamic_index_assign_rhs, frame.dynamic_index_assign_rhs_cursor)
			frame.dynamic_index_assign_rhs_cursor += 1
			working := value.clone_value(&frame.input)
			if !template_ok || value.kind_of(&working) == .Invalid { _ = value.destroy_value(&template); _ = value.destroy_value(&working); return begin_terminal_misuse(storage, .Malformed_Program) }
			dynamic_assign_suppressed := false
			for key_index in 0..<key_length {
				key, key_ok := value.array_element_copy(&frame.dynamic_index_assign_keys, key_index)
				replacement := value.clone_value(&template)
				if !key_ok || value.kind_of(&replacement) == .Invalid { _ = value.destroy_value(&key); _ = value.destroy_value(&replacement); _ = value.destroy_value(&template); _ = value.destroy_value(&working); return begin_terminal_misuse(storage, .Malformed_Program) }
				runtime_error, runtime_error_ok := dynamic_index_assign_runtime_error(&working, &key, instruction.span, storage.allocator)
				if runtime_error_ok {
					_ = value.destroy_value(&key); _ = value.destroy_value(&replacement); _ = value.destroy_value(&template); _ = value.destroy_value(&working)
					result, ready := raise_runtime(storage, index, runtime_error)
					...
					if ready do return result
					dynamic_assign_suppressed = true
					break
				}
				path, path_error := value.array_value(storage.allocator)
				if value.array_error_kind(&path_error) != .None { _ = value.destroy_array_error(&path_error); _ = value.destroy_value(&key); _ = value.destroy_value(&replacement); _ = value.destroy_value(&template); _ = value.destroy_value(&working); return resource_step(.Out_Of_Memory) }
				if _, append_error := value.array_append_take(&path, &key); value.array_error_kind(&append_error) != .None { _ = value.destroy_array_error(&append_error); _ = value.destroy_value(&path); _ = value.destroy_value(&replacement); _ = value.destroy_value(&template); _ = value.destroy_value(&working); return resource_step(.Out_Of_Memory) }
				updated, updated_ok := set_path_value(&working, &path, 0, &replacement, storage.allocator)
				_ = value.destroy_value(&path); _ = value.destroy_value(&replacement)
				if !updated_ok { _ = value.destroy_value(&template); _ = value.destroy_value(&working); return begin_terminal_misuse(storage, .Malformed_Program) }
				_ = value.destroy_value(&working); working = updated
			}
			if dynamic_assign_suppressed do continue
			_ = value.destroy_value(&template)
			result, ready := propagate_output(storage, index, &working); if ready do return result
		...
		case .Iterator_Update_Active:
			...
			item: value.Value
			if value.kind_of(&frame.input) == .Array {
				item, child_ok = value.array_element_copy(&frame.input, frame.iterator_cursor)
			} else {
				key: value.Value
				key, item, child_ok = value.object_entry_copy(&frame.input, frame.iterator_cursor)
				_ = value.destroy_value(&key)
			}
		...
		case .Iterator_Update_Delete_Active:
			...
			if cleanup_error != nil do return resource_step(cleanup_error)
			if !delete_ok do return begin_terminal_misuse(storage, .Malformed_Program)
			frame.iterator_cursor -= 1
			frame.iterator_update_seen = false
			frame.phase = .Iterator_Update_Active
			continue
		...
		case .Paths_Active, .Path_Active:
			length, length_ok := value.array_length(&frame.paths_results)
			if !length_ok do return begin_terminal_misuse(storage, .Malformed_Program)
			if frame.paths_cursor >= length { frame.phase = .Complete; continue }
			output, output_ok := value.array_element_copy(&frame.paths_results, frame.paths_cursor)
			if !output_ok do return begin_terminal_misuse(storage, .Malformed_Program)
			frame.paths_cursor += 1
			result, ready := propagate_output(storage, index, &output)
