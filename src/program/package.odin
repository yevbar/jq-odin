...
opcode_is_binary :: proc(opcode: Opcode) -> bool {
	...
	case .Add, .Subtract, .Multiply, .Divide, .Modulo,
		...
		return true
	case .Identity, .Last, .First, .Log10, .Log2, .Exp, .Exp2, .Exp10, .Asin, .Acos, .Cos, .Sin, .Tan, .Sinh, .Isinfinite, .Any_Not, .All_Not, .Error, .Try, .IsEmpty, .Range, .Limit, .Skip, .Nth, .Map, .Strftime, .Strptime, .Mktime, .Gmtime, .Fromdate, .Todate, .Negate, .Field, .Index, .Parenthesized, .Sequence, .Fork, .Optional,
...
instruction_child_count :: proc(program: ^Program, instruction: Instruction) -> Count {
	...
	case .Flatten:
		return instruction.operands_count
	case .Join, .Contains, .Split, .Index_Builtin, .Rindex_Builtin, .Indices_Builtin, .Startswith, .Endswith, .Has, .Bsearch, .Ltrimstr, .Rtrimstr, .Trimstr, .Error, .IsEmpty, .Map, .Strftime, .Strptime:
		return 1
	case .Try:
		return 2
	case .Range:
