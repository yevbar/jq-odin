...
Opcode :: enum u8 {
	...
	Binding,
	Reduce,
	// Foreach is appended to preserve existing serialized opcodes.
	Foreach,
	Length,
	Keys,
	Type,
	...
	Sort,
	// Sort_By_Key is an operand-free stable key comparator for the bounded
	// static sort_by(.field) driver lowering.
	Sort_By_Key,
	// Group_By_Key is appended to preserve existing serialized opcodes.
	Group_By_Key,
	...
	Toboolean,
	// Builtins is appended to preserve existing serialized opcodes.
	Builtins,
	Debug,
	Input,
	...
	Sinh,
	// Cosh is appended to preserve existing serialized opcodes.
	Cosh,
	// Acosh is appended to preserve existing serialized opcodes.
	Acosh,
	// Asinh is appended to preserve existing serialized opcodes.
	Asinh,
	// Atanh is appended to preserve existing serialized opcodes.
	Atanh,
	// If is appended to preserve existing serialized opcodes.
	If,
	// Isinfinite is appended to preserve existing serialized opcodes.
	Isinfinite,
	// Log is appended to preserve existing serialized opcodes.
	Log,
	// Pow is appended to preserve existing serialized opcodes.
	Pow,
	...
	Range,
	// In and Inside are appended to preserve existing serialized opcodes.
	In,
	Inside,
	...
	// Static_Field_Set_Number is appended to preserve existing serialized opcodes.
	...
	// Static_Slice_Set_Number is appended to preserve existing serialized opcodes.
	...
	Getpath,
	Setpath,
	// Path_Assign updates every path emitted by a path child with a literal RHS.
	Path_Assign,
	Delpaths,
	...
	Static_Field_Update,
	// Static_Field_Index_Update is a resumable `.name[index] |= FILTER` update.
	Static_Field_Index_Update,
	// Static_Index_Field_Update is a resumable `.[index].name |= FILTER` update.
	Static_Index_Field_Update,
	// Static_Field_Index_Field_Update is a resumable `.name[index].name |= FILTER` update.
	Static_Field_Index_Field_Update,
	// Static_Iterator_Update is a resumable root `.[] |= FILTER` update.
	...
	// Parameter_Path_Update carries an explicit RHS filter for a future
	// filter-valued callable path continuation.
	Parameter_Path_Update,
	// Alternation retains original producer, branch descriptors, and body.
	Alternation,
	Alternation_Branch,
	Pattern_Descriptor,
	// Parameter_Reference marks a formal filter reference. It is retained in
	// the program ABI while closure activation remains a later evaluator phase.
	Parameter_Reference,
	// Binding_Path_Assign retains a binding-bearing LHS for staged path capture.
	Binding_Path_Assign,
}
...
instruction_structure_valid :: proc(program: ^Program, instruction: Instruction, next_text: ^u64) -> bool {
	...
	case .Call:
		if count != 1 && count != 2 { return false }; expected_count = count
	case .Alternation:
		if count < 3 { return false }; expected_count = count
...
instruction_child_count :: proc(program: ^Program, instruction: Instruction) -> Count {
	...
	case .Static_Iterator_Update:
		return 1
	case .Static_Field_Add_Field, .Static_Field_Optional_Identity, .Static_Field_Delete:
		return 0
	case .Static_Field_Update:
		return 1
	case .Static_Field_Index_Update:
