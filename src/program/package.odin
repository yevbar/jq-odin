...
opcode_is_binary :: proc(opcode: Opcode) -> bool {
	...
	case .Add, .Subtract, .Multiply, .Divide, .Modulo,
		...
		return true
	case .Identity, .Last, .First, .Log10, .Log2, .Exp, .Exp2, .Exp10, .Asin, .Isinfinite, .Field, .Index, .Parenthesized, .Sequence, .Fork, .Optional,
...
instruction_structure_valid :: proc(program: ^Program, instruction: Instruction, next_text: ^u64) -> bool {
	...
	case .Flatten:
		...
		expected_count = count
	case .Last, .First, .Log10, .Log2, .Exp, .Exp2, .Exp10, .Asin, .Isinfinite:
		expected_count = 0
...
instruction_child_count :: proc(program: ^Program, instruction: Instruction) -> Count {
	switch instruction.opcode {
	case .Identity, .Last, .First, .Log10, .Log2, .Exp, .Exp2, .Exp10, .Asin, .Isinfinite, .Length, .Keys, .Keys_Unsorted, .Tostring, .Tonumber, .Min, .Max, .Toboolean, .Base64, .Base64d, .Uri, .Urid, .Html, .Text, .Json, .Csv, .Tsv, .Sh, .Tojson, .Fromjson, .Log, .From_Entries, .To_Entries, .Isnan, .Utf8bytelength, .Not_Builtin, .Empty, .Values, .Arrays, .Objects, .Iterables, .Scalars, .Booleans, .Nulls, .Numbers, .Strings, .Finites, .Normals, .Floor, .Round, .Transpose, .Unique, .Sort, .Type, .Abs, .Sqrt, .Fabs, .Add_Builtin, .Trim, .Ltrim, .Rtrim, .Atan, .Ascii_Downcase, .Ascii_Upcase, .Reverse, .Implode, .Explode, .Ceil, .Nan, .Infinite, .Any, .All, .Isfinite, .Isnormal:
		return 0
	case .Flatten:
