...
opcode_is_binary :: proc(opcode: Opcode) -> bool {
	...
	case .Add, .Subtract, .Multiply, .Divide, .Modulo,
		...
		return true
	case .Pow, .Identity, .If, .Last, .First, .Log10, .Log2, .Exp, .Exp2, .Exp10, .Asin, .Acos, .Cos, .Sin, .Tan, .Sinh, .Cosh, .Acosh, .Asinh, .Isinfinite, .Any_Not, .All_Not, .Error, .Try, .IsEmpty, .Range, .Limit, .Skip, .Nth, .Map, .Map_Values, .Slice, .Recurse, .Static_Field_Add_Number, .Static_Field_Set_Number, .Static_Index_Set_Number, .Strftime, .Strptime, .Mktime, .Gmtime, .Fromdate, .Todate, .Negate, .Field, .Index, .Parenthesized, .Sequence, .Fork, .Optional,
	     .In, .Inside,
...
instruction_structure_valid :: proc(program: ^Program, instruction: Instruction, next_text: ^u64) -> bool {
	...
	case .Last, .First:
		...
		expected_count = count
	case .Log10, .Log2, .Exp, .Exp2, .Exp10, .Asin, .Acos, .Cos, .Sin, .Tan, .Sinh, .Cosh, .Acosh, .Asinh, .Isinfinite, .Mktime, .Gmtime, .Fromdate, .Todate:
		expected_count = 0
