...
instruction_structure_valid :: proc(program: ^Program, instruction: Instruction, next_text: ^u64) -> bool {
	...
	for offset in 0..<count {
		...
		if instruction.opcode == .Static_Field_Add_Number || instruction.opcode == .Static_Field_Set_Number || instruction.opcode == .Static_Field_Add_Field || instruction.opcode == .Static_Field_Optional_Identity || instruction.opcode == .Static_Field_Delete || (instruction.opcode == .Static_Field_Update && offset == 0) || (instruction.opcode == .Static_Field_Index_Update && offset < 2) || instruction.opcode == .Static_Iterator_Set_Number || (instruction.opcode == .Static_Iterator_Update && offset != 0) || instruction.opcode == .Static_Index_Set_Number || instruction.opcode == .Static_Slice_Set_Number || (instruction.opcode == .Dynamic_Field_Set && offset == 0) ||
