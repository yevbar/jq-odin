...
module_definition_at :: proc(input: string, at: int, definitions: [dynamic]module_definition, namespace: string = "") -> int {
	...
	if dollar_qualified && !qualified {
		// A plain `$name` is always a jq variable. Only the explicitly
		// qualified `$alias::name` spelling can refer to a module definition.
		return -1
	}
...
module_expand_source :: proc(
	...
	for at < len(input) {
		...
		// An identifier followed by a colon is an object key, not a filter
		// expression. Keep it literal even when a parameter has the same name;
		// only the value expression after the colon is substituted.
		object_key := next < len(input) && input[next] == ':'
		parameter_index := module_parameter(parameters, name)
		bare_identifier := start == 0 || (input[start-1] != '$' && input[start-1] != '.' && input[start-1] != '@')
		if bare_identifier && !object_key && parameter_index >= 0 && parameter_index < arg_count {
		...
		for stack_previous in stack^[:depth] {
			if stack_previous == index {
				for expanded_arg in expanded_args[:expanded_arg_count] do delete(expanded_arg, allocator)
				// Re-entering a jq function is ordinary recursion, not a module
				// dependency cycle. Preserve the call for the jq compiler rather
				// than converting it into a module-loader error.
				if !module_write(builder, input[start:at]) do return {kind = .Read_Failure, resource_error = .Out_Of_Memory}
				reentered = true
				break
			}
		}
		if reentered do continue
		stack^[depth] = index
