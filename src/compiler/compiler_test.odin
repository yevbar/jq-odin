...
every_parser_node_kind_has_an_exact_completed_payload_shape :: proc(t: ^testing.T) {
	parser: syntax.Parser
	source := diagnostic.borrow_source("<shape>", `null,true,false,1,"",-.,(.)?,.a|.,.[1],1 as $x | $x,reduce . as $r (0; .),length,keys,keys_unsorted,tostring,tonumber,toboolean,@base64,@base64d,@uri,@urid,@html,@text,@json,@csv,@tsv,@sh,tojson,fromjson,log,log10,log2,exp,exp2,exp10,asin,acos,cos,sin,tan,sinh,last,first,isinfinite,min,max,from_entries,to_entries,isnan,utf8bytelength,not,empty,values,arrays,objects,iterables,scalars,booleans,nulls,numbers,strings,finites,normals,floor,round,trunc,transpose,unique,sort,ceil,flatten,nan,infinite,any,all,any(not),all(not),isfinite,isnormal,join("a"),contains("a"),split(", "),index("a"),rindex("a"),indices("a"),startswith("a"),endswith("a"),ltrimstr("a"),rtrimstr("a"),trimstr("a"),has("a"),bsearch(1),error("foo"),try error("foo") catch .,isempty(empty),limit(1; range(2)),skip(1; range(2)),nth(1; range(2)),map(.),range(0;1),strftime("%Y-%m-%dT%H:%M:%SZ"),strptime("%Y-%m-%dT%H:%M:%SZ"),mktime,gmtime,fromdateiso8601,todateiso8601,fromdate,todate,type,abs,sqrt,fabs,add,trim,ltrim,rtrim,atan,ascii_downcase,ascii_upcase,reverse,implode,explode`)
	testing.expect(t, syntax.init_parser(&parser, source, context.allocator))
	parsed := syntax.parse_filter(&parser)
	testing.expect_value(t, parsed.kind, syntax.Parse_Outcome_Kind.Success)
