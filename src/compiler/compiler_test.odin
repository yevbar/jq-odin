...
every_parser_node_kind_has_an_exact_completed_payload_shape :: proc(t: ^testing.T) {
	parser: syntax.Parser
	source := diagnostic.borrow_source("<shape>", `null,true,false,1,"",-2,(.)?,.a|.,.[1],1 as $x | $x,reduce . as $r (0; .),length,keys,keys_unsorted,tostring,from_entries,to_entries,isnan,utf8bytelength,not,empty,values,type,abs,sqrt,fabs,add,trim,ltrim,rtrim,atan,ascii_downcase,ascii_upcase,reverse,implode,explode`)
	testing.expect(t, syntax.init_parser(&parser, source, context.allocator))
	parsed := syntax.parse_filter(&parser)
	testing.expect_value(t, parsed.kind, syntax.Parse_Outcome_Kind.Success)
