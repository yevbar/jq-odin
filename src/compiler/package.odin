...
node_payload_shape_valid :: proc(node: syntax.Node) -> bool {
	...
	case .Null:
		return no_child && no_edges && no_name && no_container_links && !node.has_value &&
		       node.value == 0 && !node.boolean_value && no_number &&
		       !node.has_string_text && string_header_absent(node.string_text)
	case .Length, .Keys, .Keys_Unsorted, .Tostring, .From_Entries, .To_Entries, .Isnan, .Not_Builtin, .Type, .Abs, .Sqrt, .Fabs, .Add_Builtin, .Trim, .Ltrim, .Rtrim, .Atan, .Ascii_Downcase, .Ascii_Upcase, .Reverse, .Implode, .Explode:
		return no_child && no_edges && no_name && no_container_links && !node.has_value &&
		       !node.boolean_value && no_number && !node.has_string_text &&
		       string_header_absent(node.string_text)
	case .Field:
...
validate_binding_scopes :: proc(nodes: []syntax.Node, id: syntax.Node_Id, source: diagnostic.Source, scopes: []diagnostic.Span, depth, budget: int) -> bool {
	...
	case .Null, .Boolean, .Number, .String, .Length, .Keys, .Keys_Unsorted, .Tostring, .From_Entries, .To_Entries, .Isnan, .Not_Builtin, .Type, .Abs, .Sqrt, .Fabs, .Add_Builtin, .Trim, .Ltrim, .Rtrim, .Atan, .Ascii_Downcase, .Ascii_Upcase, .Reverse, .Implode, .Explode:
		return true
	}
	return true
}
