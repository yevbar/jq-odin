	...
	for {
		if !term_ready {
			...
			case .Format:
				...
				if format != "@base64" && format != "@base64d" && format != "@uri" && format != "@urid" && format != "@html" && format != "@text" && format != "@json" && format != "@csv" {
					fail_from_lookahead(parser, .Expression)
					return {}, false
				}
