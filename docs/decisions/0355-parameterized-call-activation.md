...
`def id(x): x; id(...)` through the syntax/compiler/evaluator path. Other
parameterized definitions continue through the mature module bridge until their
lexical binding, multi-parameter arity, and generator semantics receive their
own contracts. This keeps the new runtime proof independent of textual
...
checks the CLI-facing route and rejects `id(1,2)`. The call-frame phase remains
