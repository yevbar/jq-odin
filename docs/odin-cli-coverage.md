...
pinned catalog now reports **435 passed, 87 failed, 0 harness errors**. The
...
`IN(generator)` path is covered by the `in-dynamic` shard; two-argument
`IN(source; s)` remains deferred as a separate two-child contract. The
